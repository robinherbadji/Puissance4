...
public class Minimax extends Algorithm {	
	...
	public int choisirCoupNRecursive() {
		...
		for (int col : this.classementColonnes()) {
			...
			if (grille_next.isCoupPossible(col)) {
				...
				if (victoireAdversaire != Constantes.COUP_NON_DEFINI) {
					...
					NegamaxRecursive nega = new NegamaxRecursive(grille_next, tourExplore);
	...
	public class NegamaxRecursive extends RecursiveTask<Double> {
		List<NegamaxRecursive> listNega = new ArrayList<NegamaxRecursive>();
		Grille grille;
		int profondeur;
		double score;
		...
		protected Double compute() {
			this.score = -negamax2(this.grille, profondeur);
			return score;
		}
		...
		private double negamax2(Grille state, int profondeur) {
	...
	public int choisirCoupThreadRecursive() {
		// Stratégie de départ
		int coup = Constantes.COUP_NON_DEFINI;
