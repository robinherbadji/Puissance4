...
public class Console extends Thread {
	...
	public void run() {
		...
		/*
		j1 = new Humain(nom_adversaire, 1);
		j2 = new IA(nom_IA, 2, algo_IA, niveauIA);
		*/
	...
	private Joueur choixJoueur(int order, Scanner entry) {
		...
		if (typeJoueur == Constantes.JOUEUR_HUMAN) {
			...
			do {
				System.out.println("Niveau de difficult� de l'IA (" + nomJoueur + ") [1-42] ?");
				System.out.print("Votre choix : ");
	...
	public void lancementTour(int tour, Joueur joueurCourant, Grille grille) {
		...
		System.out.println("C'est � " + joueurCourant.getNom() + " de jouer !");
	...
	public void afficherCoup(Joueur joueurCourant, int coup, long t) {
		System.out.println(joueurCourant.getNom() + " a choisi de mettre un jeton dans la colonne " + (coup + 1)
				+ " apr�s " + timeToString(t) + " de r�flexion\n");
